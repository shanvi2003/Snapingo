...
import { Calendar, Car, CheckCircle2, ChevronLeft, MapPin, X } from "lucide-react";
...
export default function CabBookingModal({ destinations }: { destinations: Destination[] }) {
  // 2.5s so the page's own entrance animations finish before this pops up.
  const [open, setOpen] = useAutoOpenOnce("snapingo-cab-flow-seen", 2500);
  ...
  const handleSend = () => {
    ...
    setSending(true);
    const lines = [
      "Hi Snapingo! I'd like to book a cab / transfer.",
      "",
      `Name: ${contact.name}`,
      `Phone: ${contact.phone}`,
      `Email: ${contact.email}`,
      `Destination: ${selectedDestination?.name ?? ""}`,
      `Pickup date: ${pickupDate || "TBD"}`,
      "",
  ...
  const canProceedStep2 = Boolean(pickupDate);
  ...
  return (
    <>
      {!open && (
        ...
        >
          <Car className="h-4.5 w-4.5 text-brand-300" />
          Book a Cab
        </button>
      )}
          ...
          >
            ...
            >
              ...
              <div className="overflow-y-auto px-4 py-4 sm:px-6 sm:py-6">
                {step === 1 && (
                  <div>
                    <p className="text-sm text-ink-900">Where do you need the cab?</p>
                ...
                {step === 2 && (
                  <div>
                    <p className="text-sm text-ink-900">When do you need the pickup?</p>
                    <div className="mt-4">
                      <label htmlFor="cab-pickup-date" className="mb-1.5 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-ink-900">
                ...
                {step === 3 && (
                  <div>
                    <p className="text-sm text-ink-900">Almost done - who should we confirm this with?</p>
                    <div className="mt-4">
                ...
                {step === 4 && (
                  <div className="flex flex-col items-center gap-3 py-6 text-center">
                    ...
                    <p className="max-w-xs text-sm text-ink-500">
                      Our travel expert will confirm your cab availability and pricing on WhatsApp
                      shortly.
                    </p>
                  </div>
                )}
