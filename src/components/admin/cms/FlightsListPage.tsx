...
export default async function FlightsListPage({ basePath = "/admin" }: { basePath?: string }) {
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link href={`${basePath}/cms/flights/new`} className="flex items-center gap-1.5 rounded-full bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-700">
              ...
              <tr key={f.id} className="border-b border-ink-50 last:border-0 hover:bg-ink-50/60">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    <Link href={`${basePath}/cms/flights/${f.id}/edit`} className="grid h-8 w-8 place-items-center rounded-full text-ink-400 hover:bg-brand-50 hover:text-brand-600" aria-label="Edit">
