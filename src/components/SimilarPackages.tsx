...
import { allPackages, type TourPackage } from "@/data/packages";
...
export default function SimilarPackages({
  ...
  type: TourPackage["type"];
}) {
  const list = useMemo(() => {
    const sameDestination = allPackages.filter(
      (p) => p.id !== currentId && p.destinationSlug === destinationSlug
    );
    const sameType = allPackages.filter(
      (p) => p.id !== currentId && p.type === type && p.destinationSlug !== destinationSlug
    );
    return [...sameDestination, ...sameType].slice(0, 8);
  }, [currentId, destinationSlug, type]);
