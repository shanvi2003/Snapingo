      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex items-start justify-between gap-3">
          <Link href={`/packages/${pkg.id}`}>
            <h3 className="font-heading text-lg font-bold text-ink-900 transition hover:text-brand-600">
              {pkg.title}
            </h3>
          </Link>
        ...
        <p className="mt-1 text-sm text-ink-900">{pkg.duration}</p>
        ...
        <div className="mt-5 flex flex-1 items-end justify-between border-t border-ink-100 pt-4">
          ...
          <Link
            ...
            className="rounded-full bg-ink-900 px-4 py-2.5 text-xs font-semibold text-white transition hover:bg-brand-600"
          >
            View Details
          </Link>
        </div>
