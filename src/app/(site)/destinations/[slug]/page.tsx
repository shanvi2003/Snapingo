...
import { getDestinationBySlug, getDestinationSlugs } from "@/lib/content/destinations";
import { getPackagesByDestinationSlug } from "@/lib/content/packages";
...
export async function generateMetadata({
  ...
  const title = `${destination.name} Packages | Snapingo`;
  return {
    title,
    ...
    openGraph: {
      ...
      images: [{ url: destination.gallery[0], width: 1200, height: 630, alt: destination.name }],
    },
    twitter: {
...
export default async function DestinationDetailPage({
  ...
  const packages = await getPackagesByDestinationSlug(slug);
  ...
  return (
    <>
      ...
      <section className="pt-16 pb-10 sm:pt-20 sm:pb-12 lg:pb-20">
      ...
      {packages.length > 0 && (
        <section id="packages" className="scroll-mt-24 bg-ink-50/60 pt-10 pb-16 sm:pt-12 sm:pb-20 lg:pt-20">
