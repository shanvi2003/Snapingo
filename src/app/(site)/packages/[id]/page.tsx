...
export async function generateMetadata({
  ...
  if (!pkg) return {};
  const title = `${pkg.title} | Snapingo`;
  const description = `${pkg.title}: a ${pkg.duration} package for ${pkg.destination}, starting at ₹${pkg.price.toLocaleString("en-IN")} per person.`;
  return {
    title,
    description,
...
export default async function PackageDetailPage({
  ...
  return (
    <>
      ...
      />
      <JsonLd data={tripPackageJsonLd(pkg)} />
